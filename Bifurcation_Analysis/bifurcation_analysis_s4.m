...
S2 = 1;   % Anti-oxidative mechanisms (e.g., age)

% Function for steady-state ROS calculation
SteadyStateROS = @(ROS, S4) ...
    k1 * (1 + S1 + d_alpha_syn * ((k3 * ROS * S3 / (k4 * S4 * K_alpha_syn))^4) / ...
    (1 + (k3 * ROS * S3 / (k4 * S4 * K_alpha_syn))^4)) - k2 * ROS * S2;

S4_vals = linspace(0.1, 3, 1000);
ROS_grid = linspace(0, 20, 5000); 

ROS_vals = nan(length(S4_vals), 3); % For up to three steady states
for i = 1:length(S4_vals)
    S4 = S4_vals(i);
    
    % Evaluate the function on the grid
    f_vals = arrayfun(@(ROS) SteadyStateROS(ROS, S4), ROS_grid);
    ...
    for j = 1:length(crossings)
        refined_roots(j) = fzero(@(ROS) SteadyStateROS(ROS, S4), ROS_grid(crossings(j)));
    end
...
figure;
hold on;
plot(S4_vals, ROS_vals(:, 1), 'k-', 'LineWidth', 1.5); % Stable lower branch
plot(S4_vals, ROS_vals(:, 2), 'r--', 'LineWidth', 1.5); % Unstable branch
plot(S4_vals, ROS_vals(:, 3), 'k-', 'LineWidth', 1.5); % Stable upper branch
xlabel('S4 (Protein clearance factor)');
